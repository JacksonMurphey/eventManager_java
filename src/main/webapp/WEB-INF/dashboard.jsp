...
<html>
    ...
    <jsp:include page="base.jsp"></jsp:include>
		<div class="container">
            ...
            <h5>Your Location: <span class="text-secondary">${user.city}, ${user.state}</h5>
			<hr />
						...
						<tr>
							...
							<c:choose>
								<c:when test="${ event.planner.id == user.id }">
									<a href="/events/${ event.id }/edit">Edit</a> |	<form class="delete-form d-inline-block" action="/events/${ event.id }" method="post">
										<input type="hidden" name="_method" value="delete" />
										<button class=" p-0 btn btn-link text-decoration-none text-danger" style="font-size:20px;">&#9746;</button>
									</form>
								</c:when>
								<c:otherwise>
									<c:choose>
						...
						<tr>
							...
							<c:choose>
								<c:when test="${ event.planner.id == user.id }">
									...
									<form class="delete-form d-inline-block" action="/events/${ event.id }" method="post">
										<input type="hidden" name="_method" value="delete" />
										<button class=" p-0 btn btn-link text-decoration-none text-danger" style="font-size:20px; text-color:black;">&#9746;</button>
									</form>
								</c:when>
								<c:otherwise>
									<c:choose>
